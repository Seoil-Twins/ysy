...
const Date = () => {
  ...
  const [detailId] = useState<number>(Number(params.detailId));
  const [detailTitle] = useState<string | undefined>(params.title);
  ...
  const getSpectificDateAPI = useCallback(async () => {
  ...
  const getDateWithTitleAPI = useCallback(async () => {
    // title를 기반으로 API 가져오기
  ...
  const getDateWithTitle = useCallback(async () => {
    const response = await getDateWithTitleAPI();
  ...
  }, [detailTitle, getDateWithTitle, getDate]);

  useEffect(() => {
    if (detailId !== -1) {
      getSpectificDateAPI();
    }
  }, [detailId, getSpectificDateAPI]);
