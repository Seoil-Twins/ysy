...
import { ICreate, Inquire, IUpdateWithController, IUpdateWithService } from "../models/inquiry.model";
...
class InquireController {
  ...
  async addInquire(data: ICreate, images?: File | File[]): Promise<string> {
    ...
    try {
      ...
      if (images instanceof Array<File>) await this.inquireImageService.createMutiple(transaction, createdInquire.inquireId, createdInquire.userId, images);
  ...
  async updateInquire(data: IUpdateWithController, images?: File | File[]): Promise<Inquire> {
    ...
    try {
      ...
      if (images) {
        ...
        else if (images instanceof File) updatedInquireImages = await this.inquireImageService.create(transaction, inquire.inquireId, inquire.userId, images);
      }
      ...
      if (error instanceof UploadError) {
        ...
        logger.error(`After updating the firebase, a db error occurred and the firebase thumbnail is deleted => ${JSON.stringify(updatedInquireImages)}`);
